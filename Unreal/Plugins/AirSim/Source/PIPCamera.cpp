...
namespace
{
bool ShouldSanitizeAnalyticCapture(const APIPCamera::ImageType ImageType)
{
    switch (ImageType) {
    case APIPCamera::ImageType::Scene:
    case APIPCamera::ImageType::Infrared:
        return false;
...
bool ShouldStripLensEffectsFromCapture(const APIPCamera::ImageType ImageType)
{
    return ImageType == APIPCamera::ImageType::Scene;
}

bool IsAirSimLensEffectBlendable(const UObject* Blendable)
{
    if (Blendable == nullptr) {
        return false;
    }

    const FString Name = Blendable->GetName();
    return Name.Contains(TEXT("CameraDistortion")) || Name.Contains(TEXT("CameraSensorNoise"));
}

void StripAirSimLensEffectsFromSceneCapture(FPostProcessSettings& PostProcessSettings)
{
    PostProcessSettings.WeightedBlendables.Array.RemoveAll(
        [](const FWeightedBlendable& Blendable) {
            return IsAirSimLensEffectBlendable(Blendable.Object);
        });

    // Keep exposure/tone settings copied from the cine camera, but remove the
    // lens-style edge treatment so AirSim RGB matches the viewport more closely.
    PostProcessSettings.bOverride_VignetteIntensity = 1;
    PostProcessSettings.VignetteIntensity = 0.0f;
    PostProcessSettings.bOverride_SceneFringeIntensity = 1;
    PostProcessSettings.SceneFringeIntensity = 0.0f;
    PostProcessSettings.bOverride_ChromaticAberrationStartOffset = 1;
    PostProcessSettings.ChromaticAberrationStartOffset = 0.0f;
}
}
...
void APIPCamera::BeginPlay()
{
    ...
    for (unsigned int image_type = 0; image_type < imageTypeCount(); ++image_type) {
        USceneCaptureComponent2D* CaptureComponent = captures_[image_type];
...
void APIPCamera::setCaptureUpdate(USceneCaptureComponent2D* capture, bool nodisplay)
{
    ...
    if (nodisplay) {
        // Snapshot captures should not retain temporal state between requests.
        capture->bAlwaysPersistRenderingState = false;
        capture->bExcludeFromSceneTextureExtents = true;
    }
    else {
        // Continuous per-frame capture is a normal stream. Preserve view state so UE5 can
        // reuse renderer allocations instead of rebuilding transient state every frame.
...
void APIPCamera::setCameraPose(const msr::airlib::Pose& relative_pose)
{
    if (ned_transform_ == nullptr) {
        UAirBlueprintLib::LogMessageString("Cannot set camera pose because NedTransform is not initialized for", TCHAR_TO_UTF8(*GetName()), LogDebugLevel::Failure);
        return;
    }
...
msr::airlib::CameraInfo APIPCamera::getCameraInfo() const
{
    ...
    if (ned_transform_ == nullptr) {
        UAirBlueprintLib::LogMessageString("Cannot query camera info because NedTransform is not initialized for", TCHAR_TO_UTF8(*GetName()), LogDebugLevel::Failure);
        camera_info.fov = camera_ ? camera_->FieldOfView : 0.0f;
...
void APIPCamera::setupCameraFromSettings(const APIPCamera::CameraSetting& camera_setting, const NedTransform& ned_transform)
{
    ...
    for (int image_type = -1; image_type < image_count; ++image_type) {
        ...
        if (image_type >= 0) { //scene capture components
            const ImageType capture_image_type = Utils::toEnum<ImageType>(image_type);
            ...
            switch (capture_image_type) {
            case ImageType::Scene:
            case ImageType::Infrared:
                updateCaptureComponentSetting(capture_image_type, captures_[image_type], render_targets_[image_type], false, pixel_format, capture_setting, ned_transform, false);
                break;

            case ImageType::Segmentation:
            case ImageType::SurfaceNormals:
                updateCaptureComponentSetting(capture_image_type, captures_[image_type], render_targets_[image_type], true, pixel_format, capture_setting, ned_transform, true);
                break;

            default:
                updateCaptureComponentSetting(capture_image_type, captures_[image_type], render_targets_[image_type], true, pixel_format, capture_setting, ned_transform, false);
                break;
            }
            if (!ShouldSanitizeAnalyticCapture(capture_image_type)) {
            ...
            copyCameraSettingsToSceneCapture(camera_, captures_[image_type], capture_image_type); //CinemAirSim
        }
        else { //camera component
            updateCameraSetting(camera_, capture_setting, ned_transform);
            applyCineCameraSetting(camera_, camera_setting.cine_camera);
...
void APIPCamera::updateCaptureComponentSetting(const ImageType image_type, USceneCaptureComponent2D* capture, UTextureRenderTarget2D* render_target,
...
{
    const bool bSanitizeAnalyticCapture = ShouldSanitizeAnalyticCapture(image_type);
    ...
    if (!bSanitizeAnalyticCapture && !std::isnan(setting.target_gamma))
        render_target->TargetGamma = setting.target_gamma;
    else if (bSanitizeAnalyticCapture)
        render_target->TargetGamma = 1.0f;
...
void APIPCamera::applyCineCameraSetting(UCineCameraComponent* camera, const CineCameraSetting& setting)
{
    if (camera == nullptr)
        return;

    if (!setting.lens_preset_name.empty())
        camera->SetLensPresetByName(FString(setting.lens_preset_name.c_str()));

    if (!setting.filmback_preset_name.empty())
        camera->SetFilmbackPresetByName(FString(setting.filmback_preset_name.c_str()));

    if (!std::isnan(setting.sensor_width))
        camera->Filmback.SensorWidth = setting.sensor_width;
    if (!std::isnan(setting.sensor_height))
        camera->Filmback.SensorHeight = setting.sensor_height;
    if (!std::isnan(setting.focal_length))
        camera->CurrentFocalLength = setting.focal_length;

    if (setting.enable_manual_focus >= 0)
        camera->FocusSettings.FocusMethod = setting.enable_manual_focus > 0 ? ECameraFocusMethod::Manual : ECameraFocusMethod::Disable;
    if (!std::isnan(setting.focus_distance))
        camera->FocusSettings.ManualFocusDistance = setting.focus_distance;
    if (!std::isnan(setting.focus_aperture))
        camera->CurrentAperture = setting.focus_aperture;

#if WITH_EDITOR
    if (setting.enable_focus_plane >= 0)
        camera->FocusSettings.bDrawDebugFocusPlane = setting.enable_focus_plane > 0;
#endif
}
...
{
    if (ned_transform_ == nullptr) {
        UAirBlueprintLib::LogMessageString("Cannot query camera pose because NedTransform is not initialized for", TCHAR_TO_UTF8(*GetName()), LogDebugLevel::Failure);
...
void APIPCamera::copyCameraSettingsToAllSceneCapture(UCameraComponent* camera)
{
    ...
    for (int image_type = image_count - 1; image_type >= 0; image_type--) {
        copyCameraSettingsToSceneCapture(camera_, captures_[image_type], static_cast<ImageType>(image_type));
    }
}

void APIPCamera::copyCameraSettingsToSceneCapture(UCameraComponent* src, USceneCaptureComponent2D* dst, ImageType capture_type)
{
    if (src && dst) {
        ...
        dst->ProjectionType = src->ProjectionMode;
        dst->FOVAngle = camera_view_info.FOV;
        if (dst->ProjectionType == ECameraProjectionMode::Orthographic) {
            dst->OrthoWidth = camera_view_info.OrthoWidth;
        }

        if (ShouldSanitizeAnalyticCapture(capture_type)) {
            return;
        }
        ...
        const FPostProcessSettings dst_pp_settings_before_copy = dst_pp_settings;
        ...
        // Keep capture-specific exposure tuning. These values come from the AirSim
        // per-camera CaptureSettings and must survive cine-camera synchronization.
        if (dst_pp_settings_before_copy.bOverride_MotionBlurAmount) {
            dst_pp_settings.bOverride_MotionBlurAmount = 1;
            dst_pp_settings.MotionBlurAmount = dst_pp_settings_before_copy.MotionBlurAmount;
        }
        if (dst_pp_settings_before_copy.bOverride_AutoExposureMethod) {
            dst_pp_settings.bOverride_AutoExposureMethod = 1;
            dst_pp_settings.AutoExposureMethod = dst_pp_settings_before_copy.AutoExposureMethod;
        }
        if (dst_pp_settings_before_copy.bOverride_AutoExposureSpeedDown) {
            dst_pp_settings.bOverride_AutoExposureSpeedDown = 1;
            dst_pp_settings.AutoExposureSpeedDown = dst_pp_settings_before_copy.AutoExposureSpeedDown;
        }
        if (dst_pp_settings_before_copy.bOverride_AutoExposureSpeedUp) {
            dst_pp_settings.bOverride_AutoExposureSpeedUp = 1;
            dst_pp_settings.AutoExposureSpeedUp = dst_pp_settings_before_copy.AutoExposureSpeedUp;
        }
        if (dst_pp_settings_before_copy.bOverride_AutoExposureMaxBrightness) {
            dst_pp_settings.bOverride_AutoExposureMaxBrightness = 1;
            dst_pp_settings.AutoExposureMaxBrightness = dst_pp_settings_before_copy.AutoExposureMaxBrightness;
        }
        if (dst_pp_settings_before_copy.bOverride_AutoExposureMinBrightness) {
            dst_pp_settings.bOverride_AutoExposureMinBrightness = 1;
            dst_pp_settings.AutoExposureMinBrightness = dst_pp_settings_before_copy.AutoExposureMinBrightness;
        }
        if (dst_pp_settings_before_copy.bOverride_AutoExposureBias) {
            dst_pp_settings.bOverride_AutoExposureBias = 1;
            dst_pp_settings.AutoExposureBias = dst_pp_settings_before_copy.AutoExposureBias;
        }
        if (dst_pp_settings_before_copy.bOverride_AutoExposureLowPercent) {
            dst_pp_settings.bOverride_AutoExposureLowPercent = 1;
            dst_pp_settings.AutoExposureLowPercent = dst_pp_settings_before_copy.AutoExposureLowPercent;
        }
        if (dst_pp_settings_before_copy.bOverride_AutoExposureHighPercent) {
            dst_pp_settings.bOverride_AutoExposureHighPercent = 1;
            dst_pp_settings.AutoExposureHighPercent = dst_pp_settings_before_copy.AutoExposureHighPercent;
        }
        if (dst_pp_settings_before_copy.bOverride_HistogramLogMin) {
            dst_pp_settings.bOverride_HistogramLogMin = 1;
            dst_pp_settings.HistogramLogMin = dst_pp_settings_before_copy.HistogramLogMin;
        }
        if (dst_pp_settings_before_copy.bOverride_HistogramLogMax) {
            dst_pp_settings.bOverride_HistogramLogMax = 1;
            dst_pp_settings.HistogramLogMax = dst_pp_settings_before_copy.HistogramLogMax;
        }

        if (ShouldStripLensEffectsFromCapture(capture_type)) {
            StripAirSimLensEffectsFromSceneCapture(dst_pp_settings);
        }
    }
}
