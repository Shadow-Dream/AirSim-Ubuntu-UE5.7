...
public:
    struct RenderParams
    {
        ...
        bool compress;
        bool read_latest_without_capture;

        RenderParams(USceneCaptureComponent2D* render_component_val,
                     UTextureRenderTarget2D* render_target_val,
                     msr::airlib::ImageCaptureBase::ImageType image_type_val,
                     bool pixels_as_float_val,
                     bool compress_val,
                     bool read_latest_without_capture_val = false)
            : render_component(render_component_val), render_target(render_target_val), image_type(image_type_val), pixels_as_float(pixels_as_float_val), compress(compress_val), read_latest_without_capture(read_latest_without_capture_val)
        {
        }
    };
    struct RenderResult
