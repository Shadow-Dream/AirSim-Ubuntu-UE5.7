...
public:
    ...
    APIPCamera* ensureExternalCamera(const std::string& camera_name);

    bool prepareExternalCameraForCapture(const std::string& camera_name, FString& OutError) const;
    ...
    void notifyImageCapturesSceneChanged() const;
...
private:
    ...
    TWeakObjectPtr<APawn> primary_streaming_pawn_;

    UPROPERTY(Transient)
    UWorldPartitionStreamingSourceComponent* primary_vehicle_streaming_source_ = nullptr;

    float predictive_streaming_source_update_accumulator_ = 0.0f;
    bool lidar_checks_done_ = false;
    bool lidar_draw_debug_points_ = false;
    static ASimModeBase* SIMMODE;

private:
    UWorldPartitionStreamingSourceComponent* ensurePrimaryVehicleStreamingSource(APawn* Pawn);
    void updatePrimaryVehicleStreamingSource(float DeltaSeconds);
    UWorldPartitionStreamingSourceComponent* ensureExternalCameraStreamingSource(APIPCamera* camera) const;
    bool prepareExternalCameraForCapture(APIPCamera* camera, FString& OutError) const;
    void setStencilIDs();
