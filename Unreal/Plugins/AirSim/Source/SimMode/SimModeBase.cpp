...
namespace
{
void DisableCustomDepthForWorld(UWorld* World)
{
    if (World == nullptr)
    {
        return;
    ...
    int32 DisabledCustomDepthCount = 0;
    int32 ResetStencilCount = 0;
    int32 NaniteComponentCount = 0;

    for (TObjectIterator<UPrimitiveComponent> It; It; ++It)
    {
        UPrimitiveComponent* Primitive = *It;
        if (!IsValid(Primitive) || Primitive->GetWorld() != World || Primitive->HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject))
        {
            continue;
        }
        ...
        const bool bHadCustomDepth = Primitive->bRenderCustomDepth;
        const bool bHadStencilValue = Primitive->CustomDepthStencilValue != 0;
        if (!bHadCustomDepth && !bHadStencilValue)
        {
            continue;
        }
        ...
        {
            bHasNaniteData = StaticMeshComponent->HasValidNaniteData();
        }
        else if (const USkinnedMeshComponent* SkinnedMeshComponent = Cast<USkinnedMeshComponent>(Primitive))
        {
            bHasNaniteData = SkinnedMeshComponent->HasValidNaniteData();
        }

        if (bHasNaniteData)
        {
            ++NaniteComponentCount;
        }

        if (bHadCustomDepth)
        {
            Primitive->SetRenderCustomDepth(false);
            ++DisabledCustomDepthCount;
        }
    ...
    if (DisabledCustomDepthCount > 0 || ResetStencilCount > 0)
    {
        UE_LOG(LogTemp, Warning, TEXT("[AirSimCustomDepthGuard] cleared custom depth on %d components, reset stencil on %d components, nanite=%d before EndPlay"), DisabledCustomDepthCount, ResetStencilCount, NaniteComponentCount);
        FlushRenderingCommands();
    }
}

void CleanupStreamingSourcesForActor(AActor* Actor)
{
    if (Actor == nullptr)
    {
        return;
    ...
    TInlineComponentArray<UWorldPartitionStreamingSourceComponent*> StreamingComponents(Actor);
    for (UWorldPartitionStreamingSourceComponent* StreamingComponent : StreamingComponents)
    {
        if (!IsValid(StreamingComponent))
        {
            continue;
        }

        StreamingComponent->DisableStreamingSource();
        if (StreamingComponent->IsRegistered())
        {
            StreamingComponent->UnregisterComponent();
        }
        StreamingComponent->DestroyComponent();
    }
}

constexpr float GPredictiveStreamingUpdateInterval = 0.15f;
constexpr float GPredictiveStreamingSpeedMinCmPerSec = 1200.0f;
constexpr float GPredictiveStreamingSpeedMaxCmPerSec = 15000.0f;

void AddPredictiveSphere(TArray<FStreamingSourceShape>& Shapes, const FVector& Location, float LoadingRangeScale)
{
...
void AddPredictiveSector(TArray<FStreamingSourceShape>& Shapes, const FVector& Location, float LoadingRangeScale, float SectorAngleDegrees)
{
    FStreamingSourceShape Shape;
    Shape.bUseGridLoadingRange = true;
    Shape.LoadingRangeScale = LoadingRangeScale;
    Shape.bIsSector = true;
    Shape.SectorAngle = SectorAngleDegrees;
    Shape.Location = Location;
    Shape.Rotation = FRotator::ZeroRotator;
    Shapes.Add(Shape);
}

TArray<FStreamingSourceShape> BuildPredictiveStreamingShapes(const float SpeedCmPerSec)
{
    const float SpeedAlpha = FMath::GetMappedRangeValueClamped(
        FVector2D(GPredictiveStreamingSpeedMinCmPerSec, GPredictiveStreamingSpeedMaxCmPerSec),
        FVector2D(0.0f, 1.0f),
        SpeedCmPerSec);

    const float RearOffset = FMath::Lerp(1800.0f, 3200.0f, SpeedAlpha);
    const float MidOffset = FMath::Lerp(3200.0f, 10000.0f, SpeedAlpha);
    const float FarOffset = FMath::Lerp(7000.0f, 22000.0f, SpeedAlpha);
    const float FlankOffset = FMath::Lerp(1600.0f, 4200.0f, SpeedAlpha);
    const float SectorOffset = FMath::Lerp(1800.0f, 7200.0f, SpeedAlpha);
    const float SectorAngle = FMath::Lerp(105.0f, 65.0f, SpeedAlpha);
    const float SectorScale = FMath::Lerp(1.02f, 1.18f, SpeedAlpha);

    TArray<FStreamingSourceShape> Shapes;
    Shapes.Reserve(7);
    AddPredictiveSphere(Shapes, FVector::ZeroVector, 0.95f);
    AddPredictiveSphere(Shapes, FVector(-RearOffset, 0.0f, 0.0f), 0.52f);
    AddPredictiveSphere(Shapes, FVector(MidOffset, 0.0f, 0.0f), 0.85f);
    AddPredictiveSphere(Shapes, FVector(FarOffset, 0.0f, 0.0f), 0.74f);
    AddPredictiveSphere(Shapes, FVector(MidOffset * 0.82f, FlankOffset, 0.0f), 0.62f);
    AddPredictiveSphere(Shapes, FVector(MidOffset * 0.82f, -FlankOffset, 0.0f), 0.62f);
    AddPredictiveSector(Shapes, FVector(SectorOffset, 0.0f, 0.0f), SectorScale, SectorAngle);
    return Shapes;
}
}
...
void ASimModeBase::BeginPlay()
{
    ...
    // World Partition maps already manage large-world streaming around runtime sources.
    // Re-centering the world origin here can pull streaming to the wrong place at startup.
    if (this->GetWorld()->GetWorldPartition() == nullptr) {
...
{
    UE_LOG(LogTemp, Warning, TEXT("[AirSimShutdown] ASimModeBase::EndPlay begin reason=%d"), static_cast<int32>(EndPlayReason));

    DisableCustomDepthForWorld(GetWorld());

    if (loading_screen_widget_ != nullptr)
    {
        loading_screen_widget_->RemoveFromParent();
        loading_screen_widget_ = nullptr;
    }

    // Stop asynchronous subsystems before tearing down the state they reference.
    stopApiServer();

    // killRecording() already issues stopRecording() internally and waits / kills
    // the worker thread if needed. Avoid redundant teardown paths during editor quit.
    FRecordingThread::killRecording();

    external_image_capture_.reset();
    ...
    if (IsValid(primary_vehicle_streaming_source_))
    {
        primary_vehicle_streaming_source_->DisableStreamingSource();
    }
    primary_vehicle_streaming_source_ = nullptr;
    primary_streaming_pawn_.Reset();
    predictive_streaming_source_update_accumulator_ = 0.0f;

    external_cameras_.clear();
    spawned_actors_.Empty();
    vehicle_sim_apis_.clear();

    UE_LOG(LogTemp, Warning, TEXT("[AirSimShutdown] ASimModeBase::EndPlay teardown finished; calling Super"));
...
void ASimModeBase::Tick(float DeltaSeconds)
{
    ...
    updatePrimaryVehicleStreamingSource(DeltaSeconds);
...
{
    UAirBlueprintLib::LogMessageString("Sim hotkeys:", "disabled (API-only mode)", LogDebugLevel::Informational);
}
...
APIPCamera* ASimModeBase::ensureExternalCamera(const std::string& camera_name)
{
    if (APIPCamera* Existing = getExternalCamera(camera_name)) {
        return Existing;
    }

    const auto& transform = getGlobalNedTransform();
    msr::airlib::AirSimSettings::CameraSetting CameraSetting;

    const auto SettingIt = getSettings().external_cameras.find(camera_name);
    if (SettingIt != getSettings().external_cameras.end()) {
        CameraSetting = SettingIt->second;
    }

    FVector Position = transform.fromLocalNed(CameraSetting.position) - transform.fromLocalNed(msr::airlib::Vector3r::Zero());
    const FTransform CameraTransform(
        FRotator(CameraSetting.rotation.pitch, CameraSetting.rotation.yaw, CameraSetting.rotation.roll),
        Position,
        FVector(1.0f, 1.0f, 1.0f));

    FActorSpawnParameters SpawnParams;
    SpawnParams.Name = FName(*FString::Printf(TEXT("external_%s"), UTF8_TO_TCHAR(camera_name.c_str())));
    SpawnParams.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn;

    APIPCamera* Camera = GetWorld()->SpawnActor<APIPCamera>(pip_camera_class, CameraTransform, SpawnParams);
    if (!Camera) {
        return nullptr;
    }

    Camera->setupCameraFromSettings(CameraSetting, transform);
    external_cameras_.insert_or_assign(camera_name, Camera);
    return Camera;
}

bool ASimModeBase::prepareExternalCameraForCapture(const std::string& camera_name, FString& OutError) const
{
    APIPCamera* Camera = const_cast<ASimModeBase*>(this)->getExternalCamera(camera_name);
    if (!Camera) {
        OutError = FString::Printf(TEXT("External camera '%s' is not configured."), UTF8_TO_TCHAR(camera_name.c_str()));
        return false;
    }

    return prepareExternalCameraForCapture(Camera, OutError);
}

UWorldPartitionStreamingSourceComponent* ASimModeBase::ensurePrimaryVehicleStreamingSource(APawn* Pawn)
{
    if (!Pawn)
    {
        return nullptr;
    ...
    UWorld* World = GetWorld();
    if (!World || World->GetWorldPartition() == nullptr)
    {
        return nullptr;
    }

    primary_streaming_pawn_ = Pawn;

    UWorldPartitionStreamingSourceComponent* StreamingSource = primary_vehicle_streaming_source_;
    if (!IsValid(StreamingSource) || StreamingSource->GetOwner() != Pawn)
    {
        StreamingSource = nullptr;

        TInlineComponentArray<UWorldPartitionStreamingSourceComponent*> StreamingComponents(Pawn);
        for (UWorldPartitionStreamingSourceComponent* Existing : StreamingComponents)
        {
            if (Existing && Existing->GetFName() == TEXT("PrimaryVehicleStreamingSource"))
            {
                StreamingSource = Existing;
                break;
            }
        }
        ...
        if (!StreamingSource)
        {
            StreamingSource = NewObject<UWorldPartitionStreamingSourceComponent>(Pawn, TEXT("PrimaryVehicleStreamingSource"));
            if (!StreamingSource)
            {
                return nullptr;
            }

            Pawn->AddInstanceComponent(StreamingSource);
    ...
    {
        StreamingSource->Shapes = BuildPredictiveStreamingShapes(Pawn->GetVelocity().Size2D());
    }

    primary_vehicle_streaming_source_ = StreamingSource;
    return StreamingSource;
}

void ASimModeBase::updatePrimaryVehicleStreamingSource(float DeltaSeconds)
{
    UWorld* World = GetWorld();
    if (!World || World->GetWorldPartition() == nullptr)
    {
    ...
    APawn* PrimaryPawn = primary_streaming_pawn_.Get();
    if (!PrimaryPawn && getApiProvider() && getApiProvider()->hasDefaultVehicle())
    {
        if (PawnSimApi* DefaultVehicle = getVehicleSimApi())
        {
            PrimaryPawn = DefaultVehicle->getPawn();
        }
    }
    ...
    UWorldPartitionStreamingSourceComponent* StreamingSource = ensurePrimaryVehicleStreamingSource(PrimaryPawn);
    if (!StreamingSource)
    {
        return;
    }

    predictive_streaming_source_update_accumulator_ += DeltaSeconds;
    if (predictive_streaming_source_update_accumulator_ < GPredictiveStreamingUpdateInterval)
    {
        return;
    }

    predictive_streaming_source_update_accumulator_ = 0.0f;
    StreamingSource->Shapes = BuildPredictiveStreamingShapes(PrimaryPawn->GetVelocity().Size2D());
}

UWorldPartitionStreamingSourceComponent* ASimModeBase::ensureExternalCameraStreamingSource(APIPCamera* Camera) const
{
    if (!Camera) {
        return nullptr;
    }
    ...
    UWorldPartitionStreamingSourceComponent* StreamingSource = Camera->FindComponentByClass<UWorldPartitionStreamingSourceComponent>();
    if (!StreamingSource) {
        StreamingSource = NewObject<UWorldPartitionStreamingSourceComponent>(Camera, TEXT("ExternalCameraStreamingSource"));
        if (!StreamingSource) {
            return nullptr;
        }

        Camera->AddInstanceComponent(StreamingSource);
        StreamingSource->Priority = EStreamingSourcePriority::Highest;
        StreamingSource->EnableStreamingSource();
        StreamingSource->RegisterComponent();
    }
...
bool ASimModeBase::prepareExternalCameraForCapture(APIPCamera* Camera, FString& OutError) const
{
    if (!Camera) {
        OutError = TEXT("External camera is null.");
        return false;
    }
    ...
    UWorldPartitionStreamingSourceComponent* StreamingSource = ensureExternalCameraStreamingSource(Camera);
    if (!StreamingSource) {
        OutError = FString::Printf(TEXT("Failed to attach World Partition streaming source to external camera '%s'."), *Camera->GetName());
        return false;
    }
    ...
    const bool bPreviousHighPriorityLoading = WorldSettings ? WorldSettings->bHighPriorityLoadingLocal : false;
    if (WorldSettings) {
    ...
    if (WorldSettings) {
        WorldSettings->bHighPriorityLoadingLocal = bPreviousHighPriorityLoading;
    }

    if (!StreamingSource->IsStreamingCompleted()) {
        OutError = FString::Printf(TEXT("World Partition is still streaming for external camera '%s'."), *Camera->GetName());
        return false;
    }
...
void ASimModeBase::notifyImageCapturesSceneChanged() const
{
    if (api_provider_ != nullptr) {
        for (msr::airlib::VehicleSimApiBase* sim_api_base : api_provider_->getVehicleSimApis()) {
            const PawnSimApi* pawn_sim_api = static_cast<const PawnSimApi*>(sim_api_base);
            if (pawn_sim_api == nullptr) {
                continue;
            }

            const UnrealImageCapture* image_capture = pawn_sim_api->getImageCapture();
            if (image_capture != nullptr) {
                image_capture->notifySceneContentChanged();
            }
        }
    }

    if (external_image_capture_ != nullptr) {
        external_image_capture_->notifySceneContentChanged();
    }
}
...
void ASimModeBase::setupVehiclesAndCamera()
{
    ...
    {
        TArray<AActor*> pawns;
        TMap<APawn*, FString> spawned_vehicle_names;
        getExistingVehiclePawns(pawns);
        ...
        if (haveUEPawns) {
            ...
            for (const auto& vehicle_setting_pair : getSettings().vehicles) {
                ...
                if (vehicle_setting.auto_create &&
                    ...
                    spawned_vehicle_names.Add(spawned_pawn, UTF8_TO_TCHAR(vehicle_setting.vehicle_name.c_str()));
        ...
        for (AActor* pawn : pawns) {
            ...
            const FString* logical_vehicle_name = spawned_vehicle_names.Find(vehicle_pawn);
            const std::string vehicle_name_override = logical_vehicle_name != nullptr
                                                          ? std::string(TCHAR_TO_UTF8(**logical_vehicle_name))
                                                          : std::string();

            auto vehicle_sim_api = createVehicleApi(vehicle_pawn, vehicle_name_override);
            std::string vehicle_name = vehicle_sim_api->getVehicleName();
    ...
    if (getApiProvider()->hasDefaultVehicle()) {
        ...
        PawnSimApi* DefaultVehicle = getVehicleSimApi();
        DefaultVehicle->possess();
        APawn* DefaultPawn = DefaultVehicle->getPawn();
        ensurePrimaryVehicleStreamingSource(DefaultPawn);
        CameraDirector->initializeForBeginPlay(getInitialViewMode(), DefaultPawn, DefaultVehicle->getCamera("fpv"), DefaultVehicle->getCamera("front_center"), DefaultVehicle->getCamera("back_center"));
    }
    else
