...
namespace
{
const TCHAR* kAirSimLabelTag = TEXT("AirSimLabel");
const TCHAR* kAirSimLabelPrefix = TEXT("AirSimLabel_");
const TCHAR* kAirSimLabelCubePath = TEXT("/Engine/BasicShapes/Cube.Cube");
const TCHAR* kAirSimLabelMaterialPath = TEXT("/Engine/EngineDebugMaterials/M_SimpleUnlitTranslucent.M_SimpleUnlitTranslucent");

FLinearColor ParseLabelColor(const std::vector<float>& color_rgba)
{
    if (color_rgba.size() != 3 && color_rgba.size() != 4)
        throw std::invalid_argument("Label color must contain 3 or 4 normalized float values");
    ...
    return FLinearColor(
        ...
        clamp(color_rgba[1]),
        clamp(color_rgba[2]),
        color_rgba.size() >= 4 ? clamp(color_rgba[3]) : 1.0f);
}

UMaterialInterface* CreateLabelMaterialInstance(
    UMaterialInterface* base_material,
    UObject* owner,
    const FLinearColor& label_color)
{
    if (base_material == nullptr || owner == nullptr)
        return base_material;

    UMaterialInstanceDynamic* dynamic_material = UMaterialInstanceDynamic::Create(base_material, owner);
    if (dynamic_material == nullptr)
        return base_material;

    dynamic_material->SetVectorParameterValue(TEXT("Color"), label_color);
    dynamic_material->SetScalarParameterValue(TEXT("Opacity"), label_color.A);
...
std::string WorldSimApi::addLabel(const Pose& pose, const Vector3r& size, const std::vector<float>& color_rgba)
{
    if (size.x() <= 0 || size.y() <= 0 || size.z() <= 0)
        throw std::invalid_argument("Label size must be positive on all axes");

    const FTransform actor_transform = simmode_->getGlobalNedTransform().fromGlobalNed(pose);
    const FLinearColor label_color = ParseLabelColor(color_rgba);

    bool spawned_label = false;
    FString label_name;

    UAirBlueprintLib::RunCommandOnGameThread([this, &size, &actor_transform, &spawned_label, &label_name, label_color]() {
        label_name = FString::Printf(TEXT("%s%s"), kAirSimLabelPrefix, *FGuid::NewGuid().ToString(EGuidFormats::Digits));

        FActorSpawnParameters spawn_params;
        spawn_params.Name = FName(*label_name);

        AActor* new_actor = createLabelActor(spawn_params, actor_transform, size, { label_color.R, label_color.G, label_color.B, label_color.A });
        if (IsValid(new_actor)) {
            spawned_label = true;
            simmode_->scene_object_map.Add(label_name, new_actor);
        }
    },
                                             true);

    if (!spawned_label)
        throw std::runtime_error("Engine could not spawn AirSim label actor");

    return TCHAR_TO_UTF8(*label_name);
}

bool WorldSimApi::destroyLabel(const std::string& label_id)
{
    bool result = false;

    UAirBlueprintLib::RunCommandOnGameThread([this, &label_id, &result]() {
        const FString label_name(label_id.c_str());
        if (!isLabelId(label_name))
            return;

        AActor* actor = simmode_->scene_object_map.FindRef(label_name);
        if (!IsValid(actor)) {
            simmode_->scene_object_map.Remove(label_name);
            return;
        }
...
uint32_t WorldSimApi::destroyAllLabels()
{
    uint32_t removed_count = 0;

    UAirBlueprintLib::RunCommandOnGameThread([this, &removed_count]() {
        TArray<FString> labels_to_remove;
        for (const TPair<FString, AActor*>& pair : simmode_->scene_object_map) {
            if (isLabelId(pair.Key))
                labels_to_remove.Add(pair.Key);
        }

        for (const FString& label_name : labels_to_remove) {
            if (AActor* actor = simmode_->scene_object_map.FindRef(label_name); IsValid(actor)) {
                actor->Destroy();
                ++removed_count;
            }
            simmode_->scene_object_map.Remove(label_name);
...
void WorldSimApi::notifyImageCapturesSceneChanged()
{
    simmode_->notifyImageCapturesSceneChanged();
}
...
AActor* WorldSimApi::createLabelActor(const FActorSpawnParameters& spawn_params, const FTransform& actor_transform, const Vector3r& size, const std::vector<float>& color_rgba)
{
    UWorld* world = simmode_->GetWorld();
    if (world == nullptr)
        return nullptr;

    UStaticMesh* cube_mesh = LoadObject<UStaticMesh>(nullptr, kAirSimLabelCubePath);
    UMaterialInterface* base_material = LoadObject<UMaterialInterface>(nullptr, kAirSimLabelMaterialPath);
    if (cube_mesh == nullptr || base_material == nullptr)
        return nullptr;

    AActor* new_actor = world->SpawnActor<AActor>(AActor::StaticClass(), FVector::ZeroVector, FRotator::ZeroRotator, spawn_params);
    if (!IsValid(new_actor))
        return nullptr;

    UStaticMeshComponent* mesh_component = NewObject<UStaticMeshComponent>(new_actor, TEXT("LabelMesh"));
    if (!IsValid(mesh_component))
        return nullptr;

    const float cube_extent_uu = 100.0f;
    const FVector world_scale(
        simmode_->getGlobalNedTransform().fromNed(size.x()) / cube_extent_uu,
        simmode_->getGlobalNedTransform().fromNed(size.y()) / cube_extent_uu,
        simmode_->getGlobalNedTransform().fromNed(size.z()) / cube_extent_uu);

    mesh_component->SetStaticMesh(cube_mesh);
    ...
    mesh_component->SetGenerateOverlapEvents(false);
    mesh_component->SetCastShadow(false);
    mesh_component->bCastDynamicShadow = false;
    mesh_component->bCastStaticShadow = false;
    mesh_component->SetReceivesDecals(false);
    mesh_component->SetMobility(EComponentMobility::Movable);
    mesh_component->SetRelativeLocation(FVector::ZeroVector);
    mesh_component->SetRelativeScale3D(world_scale);
    mesh_component->SetHiddenInGame(false, true);
    new_actor->SetRootComponent(mesh_component);
    new_actor->AddInstanceComponent(mesh_component);
    mesh_component->RegisterComponent();

    const FLinearColor label_color = ParseLabelColor(color_rgba);
    UMaterialInterface* label_material = CreateLabelMaterialInstance(base_material, mesh_component, label_color);
    mesh_component->SetMaterial(0, label_material);

    new_actor->Tags.Add(kAirSimLabelTag);
    new_actor->SetActorEnableCollision(false);
    new_actor->SetActorLocationAndRotation(actor_transform.GetLocation(), actor_transform.GetRotation(), false, nullptr, ETeleportType::TeleportPhysics);
    return new_actor;
}

bool WorldSimApi::isLabelId(const FString& object_name)
{
    return object_name.StartsWith(kAirSimLabelPrefix);
}

bool WorldSimApi::isLabelActor(const AActor* actor)
{
    return actor != nullptr && actor->Tags.Contains(kAirSimLabelTag);
}
...
bool WorldSimApi::runConsoleCommand(const std::string& command)
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &command, &succeeded]() {
        ...
        const FString trimmed_command = fStringCommand.TrimStartAndEnd();

        TArray<FString> tokens;
        trimmed_command.ParseIntoArrayWS(tokens);
        if (tokens.Num() > 0) {
            const FString& first_token = tokens[0];
            if (first_token.Equals(TEXT("py"), ESearchCase::IgnoreCase) ||
                first_token.Equals(TEXT("python"), ESearchCase::IgnoreCase) ||
                first_token.Equals(TEXT("pythonscript"), ESearchCase::IgnoreCase)) {
                UE_LOG(LogTemp, Warning, TEXT("[AirSimConsoleGuard] Rejected runtime Python console command via simRunConsoleCommand: %s"), *trimmed_command.Left(160));
                succeeded = false;
                return;
            }
        }

        succeeded = UAirBlueprintLib::RunConsoleCommand(simmode_, trimmed_command);
    },
                                             true);
    return succeeded;
}
...
bool WorldSimApi::testLineOfSightBetweenPoints(const msr::airlib::GeoPoint& lla1, const msr::airlib::GeoPoint& lla2) const
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &lla1, &lla2, &hit]() {
        ...
        if (settings.show_los_debug_lines_) {
            ...
            DrawDebugLine(simmode_->GetWorld(), point1, point2, color.ToFColor(true), true, 999999.0f, 0, 4.0f);
        }
    },
                                             true);
...
msr::airlib::CameraInfo WorldSimApi::getCameraInfo(const CameraDetails& camera_details) const
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, camera, camera_details, &info]() {
        info = camera->getCameraInfo();
        if (camera_details.external && camera != nullptr) {
            info.pose = simmode_->getGlobalNedTransform().toGlobalNed(camera->GetActorTransform());
        }
    },
                                             true);
...
void WorldSimApi::setCameraPose(const msr::airlib::Pose& pose, const CameraDetails& camera_details)
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, camera, camera_details, &pose]() {
        if (camera_details.external && camera != nullptr) {
            const FTransform camera_transform = simmode_->getGlobalNedTransform().fromGlobalNed(pose);
            camera->SetActorLocationAndRotation(
                camera_transform.GetLocation(),
    ...
    const std::vector<ImageCaptureBase::ImageRequest>& requests, const std::string& vehicle_name, bool external) const
{
    ...
    if (external) {
        std::set<std::string> PreparedCameraNames;
        FString Error;
        for (const ImageCaptureBase::ImageRequest& Request : requests) {
            if (PreparedCameraNames.find(Request.camera_name) != PreparedCameraNames.end()) {
                continue;
            }

            if (!simmode_->prepareExternalCameraForCapture(Request.camera_name, Error)) {
                responses.resize(requests.size());
                const std::string ErrorMessage = TCHAR_TO_UTF8(*Error);
                for (ImageCaptureBase::ImageResponse& Response : responses) {
            ...
            PreparedCameraNames.insert(Request.camera_name);
        }
    }
