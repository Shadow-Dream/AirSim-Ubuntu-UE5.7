...
public class AirSim : ModuleRules
{
    ...
    private bool SupportsMavLink(ReadOnlyTargetRules Target)
    {
        return Target.Platform == UnrealTargetPlatform.Win64;
    }

    private bool AddRpcDependency(ReadOnlyTargetRules Target)
    {
        if (Target.Platform == UnrealTargetPlatform.Linux)
        {
            AddEngineThirdPartyPrivateStaticDependencies(Target, "RPCLib");
            PublicDefinitions.Add("WITH_RPCLIB_BINDING=1");
            return true;
        }

        return LoadAirSimDependency(Target, "rpclib", "rpc");
    }

    private void SetupCompileMode(CompileMode mode, ReadOnlyTargetRules Target)
    {
        if (SupportsMavLink(Target))
        {
            LoadAirSimDependency(Target, "MavLinkCom", "MavLinkCom");
        }
        else
        {
            PublicDefinitions.Add("WITH_MAVLINKCOM_BINDING=0");
        }
    ...
    public AirSim(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        PublicDependencyModuleNames.AddRange(new string[] { "Core", "CoreUObject", "Engine", "InputCore", "ImageWrapper", "RenderCore", "RHI", "PhysicsCore", "AssetRegistry", "ChaosVehicles", "Landscape", "CinematicCamera", "Json" });
