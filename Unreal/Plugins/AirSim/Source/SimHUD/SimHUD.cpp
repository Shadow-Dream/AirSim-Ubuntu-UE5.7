...
void ASimHUD::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    ...
    // During PIE shutdown / editor quit, the engine is already tearing down the
    // widget tree and world actor graph. Forcing manual Destruct()/Destroy() here
    // risks running overlapping teardown paths and has been a strong suspect for
    // the remaining invalid-free crash during CloseEditor() after PIE stop.
    if (widget_) {
...
void ASimHUD::updateWidgetSubwindowVisibility()
{
    for (int window_index = 0; window_index < AirSimSettings::kSubwindowCount; ++window_index) {
        ...
        if (camera != nullptr) {
            ...
            // Hidden subwindows must leave the capture in on-demand snapshot mode. Otherwise the
            // default camera can be switched into continuous rendering even when no subwindow is shown.
            camera->setCameraTypeUpdate(camera_type, !is_visible);
        }
...
void ASimHUD::createMainWidget()
{
    ...
    if (widget_class_ != nullptr) {
        ...
        APawn* pawn = player_controller ? player_controller->GetPawn() : nullptr;
        if (pawn) {
...
{
    UAirBlueprintLib::LogMessageString("SimMode default:", AirSimSettings::kSimModeTypeMultirotor, LogDebugLevel::Informational);
