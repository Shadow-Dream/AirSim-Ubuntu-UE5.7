...
public:
    ...
    virtual std::string addLabel(const Pose& pose, const Vector3r& size, const std::vector<float>& color_rgba) override;
    virtual bool destroyLabel(const std::string& label_id) override;
    virtual uint32_t destroyAllLabels() override;
    virtual void notifyImageCapturesSceneChanged() override;
...
private:
    ...
    AActor* createLabelActor(const FActorSpawnParameters& spawn_params, const FTransform& actor_transform, const Vector3r& size, const std::vector<float>& color_rgba);
    static bool isLabelId(const FString& object_name);
    static bool isLabelActor(const AActor* actor);
    void spawnPlayer();
