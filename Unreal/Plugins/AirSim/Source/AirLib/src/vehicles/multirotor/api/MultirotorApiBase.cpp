...
namespace airlib
{
    ...
    bool MultirotorApiBase::land(float timeout_sec)
    {
        ...
        // Don't declare landing complete just because vertical speed briefly approaches zero at altitude.
        // Require the simulator to report Landed and then stay settled for a few control ticks.
        int landed_stable_count = 0;

        return waitForFunction([&]() {
                   if (getLandedState() == LandedState::Landed) {
                       commandVelocity(0, 0, 0, YawMode::Zero());

                       if (getVelocity().norm() <= approx_zero_vel_)
                           ++landed_stable_count;
                       else
                           landed_stable_count = 0;

                       return landed_stable_count > 10;
                   }

                   landed_stable_count = 0;
                   const float altitude_above_home = std::max(0.0f, -getPosition().z());
                   float descent_velocity = landing_vel_;
                   if (altitude_above_home > 20.0f)
                       descent_velocity = 3.0f;
                   else if (altitude_above_home > 5.0f)
                       descent_velocity = 1.5f;
                   else
                       descent_velocity = 0.5f;

                   moveByVelocityInternal(0, 0, descent_velocity, YawMode::Zero());
                   return false;
               },
                               timeout_sec)
            .isComplete();
    ...
    bool MultirotorApiBase::moveByVelocityZ(float vx, float vy, float z, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        ...
        {
            std::lock_guard<std::mutex> guard(velocity_z_stream_mutex_);
            velocity_z_stream_.vx = vx;
            velocity_z_stream_.vy = vy;
            velocity_z_stream_.z = z;
            velocity_z_stream_.yaw_mode = adj_yaw_mode;
            velocity_z_stream_.deadline = std::chrono::steady_clock::now() +
                                        std::chrono::duration_cast<std::chrono::steady_clock::duration>(std::chrono::duration<float>(duration));
            velocity_z_stream_.command_pending = true;
            ++velocity_z_stream_.seq;

            if (!velocity_z_stream_.stream_running) {
                velocity_z_stream_.stream_running = true;
                become_owner = true;
            }
        }
        ...
        while (!getCancelToken().isCancelled()) {
            VelocityZCommandState snapshot;
            {
                std::lock_guard<std::mutex> guard(velocity_z_stream_mutex_);
                snapshot = velocity_z_stream_;
            }
            ...
            if (now <= snapshot.deadline) {
                moveByVelocityZInternal(snapshot.vx, snapshot.vy, snapshot.z, snapshot.yaw_mode);
                std::this_thread::sleep_for(std::chrono::duration<float>(getCommandPeriod()));
                continue;
            }

            // Preserve altitude by zeroing XY velocity while holding the most recent Z target
            // once the latest streamed command expires.
            moveByVelocityZInternal(0.0f, 0.0f, snapshot.z, snapshot.yaw_mode);

            bool finished = false;
            {
                std::lock_guard<std::mutex> guard(velocity_z_stream_mutex_);
                if (velocity_z_stream_.command_pending &&
                    velocity_z_stream_.seq == snapshot.seq &&
                    std::chrono::steady_clock::now() > velocity_z_stream_.deadline) {
                    velocity_z_stream_.command_pending = false;
                    velocity_z_stream_.stream_running = false;
                    finished = true;
                }
            }
    ...
    {
        Waiter waiter(getCommandPeriod(), timeout_sec, getCancelToken());
        if (timeout_sec <= 0 || getCancelToken().isCancelled())
            return waiter;
