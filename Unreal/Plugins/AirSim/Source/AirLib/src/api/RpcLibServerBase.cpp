...
namespace airlib
{
        ...
        : api_provider_(api_provider)
    {
        ...
        pimpl_->server.bind("simAddLabel", [&](const RpcLibAdaptorsBase::Pose& pose, const RpcLibAdaptorsBase::Vector3r& size, const vector<float>& color_rgba) -> string {
            return getWorldSimApi()->addLabel(pose.to(), size.to(), color_rgba);
        });

        pimpl_->server.bind("simDestroyLabel", [&](const string& label_id) -> bool {
            return getWorldSimApi()->destroyLabel(label_id);
        });

        pimpl_->server.bind("simDestroyAllLabels", [&]() -> uint32_t {
            return getWorldSimApi()->destroyAllLabels();
        });

        pimpl_->server.bind("simNotifyImageCapturesSceneChanged", [&]() -> void {
            getWorldSimApi()->notifyImageCapturesSceneChanged();
        });
