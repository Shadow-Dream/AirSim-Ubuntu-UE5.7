    ...
    namespace msr
{
    namespace airlib
    {
        ...
        std::string RpcLibClientBase::simAddLabel(const Pose& pose, const Vector3r& size, const vector<float>& color_rgba)
        {
            return pimpl_->client.call("simAddLabel", RpcLibAdaptorsBase::Pose(pose), RpcLibAdaptorsBase::Vector3r(size), color_rgba).as<std::string>();
        }

        bool RpcLibClientBase::simDestroyLabel(const std::string& label_id)
        {
            return pimpl_->client.call("simDestroyLabel", label_id).as<bool>();
        }

        uint32_t RpcLibClientBase::simDestroyAllLabels()
        {
            return pimpl_->client.call("simDestroyAllLabels").as<uint32_t>();
        }

        void RpcLibClientBase::simNotifyImageCapturesSceneChanged()
        {
            pimpl_->client.call("simNotifyImageCapturesSceneChanged");
        }
