...
public:
    ...
    virtual void update() override
    {
        ...
        // For moveByVelocityZ-style goals, Axis4 stores horizontal velocity in x/y and a world-position target in val4.
        // The previous implementation converted that full [vx, vy, z_target] tuple into body frame for the horizontal
        // controllers, which caused the large world-Z position target to leak into local X/Y whenever the vehicle tilted.
        // That effectively cancelled most XY authority at altitude and showed up as very weak moveByVelocityZ response,
        // especially after API release/reacquire. Horizontal velocity control should use only horizontal velocity goals,
        // while the vertical controller should stay in world Z.
        const Axis3r& full_goal_velocity_world = Axis4r::axis4ToXyz(
            goal_->getGoalValue(), true);
        const Axis3r& measured_velocity_world = state_estimator_->getLinearVelocity();

        if (axis_ == 3) {
            pid_->setGoal(full_goal_velocity_world.z());
            pid_->setMeasured(measured_velocity_world.z());
        }
        else {
            const Axis3r horizontal_goal_velocity_world(full_goal_velocity_world.x(),
                                                        full_goal_velocity_world.y(),
                                                        0);
            const Axis3r horizontal_measured_velocity_world(measured_velocity_world.x(),
                                                            measured_velocity_world.y(),
                                                            0);
            const Axis4r goal_velocity_local = Axis4r::xyzToAxis4(
                state_estimator_->transformToBodyFrame(horizontal_goal_velocity_world), true);
            const Axis4r measured_velocity_local = Axis4r::xyzToAxis4(
                state_estimator_->transformToBodyFrame(horizontal_measured_velocity_world), true);
