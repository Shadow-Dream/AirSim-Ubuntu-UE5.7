...
namespace airlib
{
        ...
        {
            {
                ...
                velocity_z_stream_.deadline = std::chrono::steady_clock::time_point::min();
    ...
    protected: //types
        ...
        public:
                ...
                : api_(api)
            {
                ...
                const uint64_t request_id = token.issueRequest();
                ...
                if (isRootCall()) {
                    if (request_id != token.getLatestRequestId())
                        token.cancel();
                    else
                        token.reset();
                }
    ...
    private: //variables
        struct VelocityZCommandState
        {
            ...
            float vy = 0.0f;
            float z = 0.0f;
            YawMode yaw_mode = YawMode::Zero();
            uint64_t seq = 0;
            std::chrono::steady_clock::time_point deadline = std::chrono::steady_clock::time_point::min();
        };

        CancelToken token_;
        std::recursive_mutex status_mutex_;
        std::mutex velocity_z_stream_mutex_;
        VelocityZCommandState velocity_z_stream_;
