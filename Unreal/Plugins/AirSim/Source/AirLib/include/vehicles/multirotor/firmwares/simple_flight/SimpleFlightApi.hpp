...
namespace airlib
{
    ...
    public: //MultirotorApiBase implementation
        ...
        virtual bool setRCData(const RCData& rc_data) override
        {
            ...
            board_->setIsLocalInputDisabled(rc_data.vendor_id == "LocalInputDisabled");
            if (rc_data.is_valid) {
        ...
        bool setGoalAndModeOrCancel(const simple_flight::Axis4r& goal, const simple_flight::GoalMode& mode)
        {
            std::string message;
            const bool accepted = firmware_->offboardApi().setGoalAndMode(&goal, &mode, message);
            if (!accepted)
                cancelLastTask();
            return accepted;
        }
    ...
    protected:
        ...
        virtual void commandMotorPWMs(float front_right_pwm, float rear_left_pwm, float front_left_pwm, float rear_right_pwm) override
        {
            ...
            setGoalAndModeOrCancel(goal, mode);
        }
