...
namespace airlib
{
    ...
    public:
        CancelToken()
            : is_cancelled_(false), is_complete_(false), recursion_count_(0), request_sequence_(0)
        {
        }
        ...
        uint64_t issueRequest()
        {
            return ++request_sequence_;
        }

        uint64_t getLatestRequestId() const
        {
            return request_sequence_.load();
        }
    ...
    private:
        ...
        std::atomic<uint64_t> request_sequence_;
