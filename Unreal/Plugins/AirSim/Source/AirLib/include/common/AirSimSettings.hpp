...
namespace airlib
{
    ...
    public: //types
        ...
        struct CineCameraSetting
        {
            std::string lens_preset_name = "";
            std::string filmback_preset_name = "";
            float sensor_width = Utils::nan<float>();
            float sensor_height = Utils::nan<float>();
            float focal_length = Utils::nan<float>();
            int enable_manual_focus = -1;
            float focus_distance = Utils::nan<float>();
            float focus_aperture = Utils::nan<float>();
            int enable_focus_plane = -1;
        };
    ...
    public: //methods
        ...
        const VehicleSetting* getVehicleSetting(const std::string& vehicle_name) const
        {
            ...
            if (vehicle_name.empty() && vehicles.size() == 1)
            ...
            if (vehicles.size() == 1) {
                Utils::log(Utils::stringf("Vehicle setting '%s' was not found; falling back to only configured vehicle '%s'",
                                          vehicle_name.c_str(), vehicles.begin()->first.c_str()),
                           Utils::kLogLevelWarn);
                return vehicles.begin()->second.get();
            }

            Utils::log(Utils::stringf("Vehicle setting '%s' was not found and no safe fallback exists", vehicle_name.c_str()),
                       Utils::kLogLevelWarn);
            return nullptr;
        }
    ...
    private:
                                                                    ...
                                                                    const CameraSetting& camera_defaults)
        {
            ...
            vehicle_setting->strict_scene_freshness_after_pose_change =
                settings_json.getBool("StrictSceneFreshnessAfterPoseChange",
                                      vehicle_setting->strict_scene_freshness_after_pose_change);
        ...
        static CameraSetting createCameraSetting(const Settings& settings_json, const CameraSetting& camera_defaults)
        {
            ...
            Settings json_cine_camera;
            if (settings_json.getChild("CineCameraSettings", json_cine_camera))
                setting.cine_camera = createCineCameraSetting(json_cine_camera, setting.cine_camera);
        ...
        static CineCameraSetting createCineCameraSetting(const Settings& settings_json, const CineCameraSetting& defaults)
        {
            CineCameraSetting setting = defaults;
            setting.lens_preset_name = settings_json.getString("LensPresetName", setting.lens_preset_name);
            setting.filmback_preset_name = settings_json.getString("FilmbackPresetName", setting.filmback_preset_name);
            setting.sensor_width = settings_json.getFloat("SensorWidth", setting.sensor_width);
            setting.sensor_height = settings_json.getFloat("SensorHeight", setting.sensor_height);
            setting.focal_length = settings_json.getFloat("FocalLength", setting.focal_length);
            setting.focus_distance = settings_json.getFloat("FocusDistance", setting.focus_distance);
            setting.focus_aperture = settings_json.getFloat("FocusAperture", setting.focus_aperture);

            if (settings_json.hasKey("EnableManualFocus"))
                setting.enable_manual_focus = settings_json.getBool("EnableManualFocus", false) ? 1 : 0;
            if (settings_json.hasKey("EnableFocusPlane"))
                setting.enable_focus_plane = settings_json.getBool("EnableFocusPlane", false) ? 1 : 0;
