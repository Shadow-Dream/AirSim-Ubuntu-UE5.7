...
namespace airlib
{
    ...
    public:
        ...
        std::string simAddLabel(const Pose& pose, const Vector3r& size, const vector<float>& color_rgba);
        bool simDestroyLabel(const std::string& label_id);
        uint32_t simDestroyAllLabels();
        void simNotifyImageCapturesSceneChanged();
