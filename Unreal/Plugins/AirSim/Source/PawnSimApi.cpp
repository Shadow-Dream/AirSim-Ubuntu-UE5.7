...
void PawnSimApi::initialize()
{
    ...
    image_capture_.reset(new UnrealImageCapture(
        &cameras_,
        getVehicleSetting()->strict_scene_freshness_after_pose_change));
                             ...
                             bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
    ...
    UAirBlueprintLib::LogMessageString("AirSim Collision | ", Utils::stringf("#%d with %s | ObjID=%d | pos_ned=(%.2f, %.2f, %.2f)", state_.collision_info.collision_count, state_.collision_info.object_name.c_str(), state_.collision_info.object_id, state_.collision_info.position.x(), state_.collision_info.position.y(), state_.collision_info.position.z()), LogDebugLevel::Failure, 4);
}
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, lla, &hit]() {
        ...
        if (AirSimSettings::singleton().show_los_debug_lines_) {
            if (hit) {
                ...
                DrawDebugLine(params_.pawn->GetWorld(), params_.pawn->GetActorLocation(), target_location, color.ToFColor(true), false, -1.0f, 0, 10.0f);
            }
            else {
...
void PawnSimApi::setPoseInternal(const Pose& pose, bool ignore_collision)
{
    ...
    if (enable_teleport)
        ...
        FHitResult sweep_hit;
        params_.pawn->SetActorLocationAndRotation(position, orientation, true, &sweep_hit, ETeleportType::None);

        if (sweep_hit.bBlockingHit && sweep_hit.GetActor() != params_.pawn) {
            const auto now_nanos = msr::airlib::ClockFactory::get()->nowNanos();
            const std::string hit_actor_name = std::string(sweep_hit.GetActor() ? TCHAR_TO_UTF8(*(sweep_hit.GetActor()->GetName())) : "(null)");
            const Vector3r hit_impact_point = ned_transform_.toLocalNed(sweep_hit.ImpactPoint);
            const bool same_actor = state_.collision_info.object_name == hit_actor_name;
            const bool close_in_time = state_.collision_info.time_stamp != 0 && (now_nanos - state_.collision_info.time_stamp) < 10000000ULL;
            const bool close_in_space = (hit_impact_point - state_.collision_info.impact_point).squaredNorm() < 0.01f;

            if (!(same_actor && close_in_time && close_in_space)) {
                onCollision(Cast<UPrimitiveComponent>(params_.pawn->GetRootComponent()),
                            sweep_hit.GetActor(),
                            sweep_hit.GetComponent(),
                            true,
                            sweep_hit.ImpactPoint,
                            sweep_hit.ImpactNormal,
                            FVector::ZeroVector,
                            sweep_hit);
            }
        }
