...
#gets the static meshes in the unreal scene
    ...
    def simAddLabel(self, pose, size, color_rgba):
        """Spawn an unlit label cube in the scene.

        Args:
            pose (airsim.Pose): Global NED pose of the label.
            size (airsim.Vector3r): Label size in meters along x/y/z.
            color_rgba (list[float]): 3 or 4 normalized floats.

        Returns:
            str: Generated label id.
        """
        return self.client.call('simAddLabel', pose, size, color_rgba)

    def simDestroyLabel(self, label_id):
        """Destroy a previously created label.

        Args:
            label_id (str): Label id returned by ``simAddLabel``.

        Returns:
            bool: True if the label existed and was queued for removal.
        """
        return self.client.call('simDestroyLabel', label_id)

    def simDestroyAllLabels(self):
        """Destroy all AirSim-managed labels.

        Returns:
            int: Number of labels removed.
        """
        return self.client.call('simDestroyAllLabels')

    def simNotifyImageCapturesSceneChanged(self):
        """Notify AirSim that scene content changed and active Scene captures should refresh."""
        return self.client.call('simNotifyImageCapturesSceneChanged')
